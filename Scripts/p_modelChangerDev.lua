...
if (not VIP_USER) then
	...
	return;
elseif ((string.find(GetGameVersion(), 'Releases/5.24') == nil) and ((string.find(GetGameVersion(), 'Releases/6.1') == nil))) then
...
if sPacket.header == 108 or sPacket.header == 117 or sPacket.header == 0xAC then return end
  if sPacket.size > 5 then
      sPacket.pos = 2;
      local nID = sPacket:DecodeF();
      --print("ASAA")
      if nID == myHero.networkID then
        if sPacket.header == 0x39 then
      print(string.format("%02X",sPacket.header));
      end
      end
  end

  if (sPacket.header == 0x9C) then
